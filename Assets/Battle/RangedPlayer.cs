...
public class RangedPlayerMove : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (hpText != null)
        {
            ...
            hpText.gameObject.SetActive(false); // 원본 숨기기
        }
    ...
    void Shoot()
    {
        ...
        bullet.GetComponent<Projectile>().Init(target,attackDamage);
    }
    ...
    {
        Debug.Log($"RangedPlayer가 {damage}의 데미지를 입었습니다!");
    }
    ...
    void OnMouseDown()
    {
        ...
        {
            StatsUIManager.instance.ToggleRangedStats(this);
            Debug.Log("원거리 플레이어 터치 감지 성공!");
        }
    }
